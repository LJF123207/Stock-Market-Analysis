...
class GATs(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fusion_type = str(kwargs.pop("fusion_type", "add")).lower()  # add concat
    ...
    def _split_feature(self, feature):
        seq_feat = feature[:, :, : self.d_feat]
        news_feat = None
        if self.usenews and self.news_dim > 0:
            news_feat = feature[:, :, self.d_feat : self.d_feat + self.news_dim]
        return seq_feat, news_feat
    ...
    def train_epoch(self, data_loader):
        ...
        for data in data_loader:
            ...
            seq_feat, news_feat = self._split_feature(feature)
            pred = self.GAT_model(seq_feat.float(), None if news_feat is None else news_feat.float())
    ...
    def predict(self, dataset, segment="test"):
        ...
        dl_test = dataset.prepare(segment, col_set=test_cols, data_key=DataHandlerLP.DK_I)
        ...
        for data in test_loader:
            ...
            with torch.no_grad():
                pred = self.GAT_model(seq_feat.float(), None if news_feat is None else news_feat.float()).detach().cpu().numpy()
...
class GATModel(nn.Module):
    def __init__(
        ...
        self.news_proj = nn.Linear(self.news_dim, self.d_feat) if self.use_news else None
        self.fusion_proj = nn.Linear(self.news_dim + self.d_feat, self.d_feat) if self.use_news and self.fusion_type == "concat" else None
    ...
    def forward(self, x, news=None):
        if self.use_news:
            if self.fusion_type == "add":
                x = x + self.news_proj(news)
            elif self.fusion_type == "concat":
                x = torch.cat([x, news], dim=-1)            # [B, T, 2*hid]
                x = self.fusion_proj(x)        
            elif self.fusion_type == "crossattn":
                for layer in self.cross_attn_layers:
                    x, _ = layer(x, news)
            elif self.fusion_type == "decoder":
                x = self.decoder_layers(x, self.news_proj(news))
        out, _ = self.rnn(x)
        hidden = out[:, -1, :]
