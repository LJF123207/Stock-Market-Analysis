...
class ALSTMModel(nn.Module):
    ...
    def _build_model(self):
        ...
        self.feature_fc = nn.Linear(in_features=self.d_feat, out_features=self.hid_size)
        if self.use_news:
            self.news_fc = nn.Linear(in_features=self.news_dim, out_features=self.hid_size)
            if self.fusion_type == "concat":
                self.fusion_proj = nn.Linear(self.hid_size + self.news_dim, self.hid_size)
        else:
            self.news_fc = None
        ...
        if self.use_news and self.fusion_type == "decoder":
            self.decoder_layers = CrossAttentionDecoder(
                ...
                dim_feedforward=4 * self.hid_size,
                dropout=0.1,
            )
    ...
    def forward(self, inputs):
        base_feat = inputs[:, :, : self.d_feat]
        proj = self.feature_fc(base_feat)
        
        if self.use_news:
            news_feat = inputs[:, :, self.d_feat : self.d_feat + self.news_dim]
            
            if self.fusion_type == "add":
                news_hidden = self.news_fc(news_feat)                  # [B, T, hid]
                proj = proj + news_hidden
            elif self.fusion_type == "concat":
                proj = torch.cat([proj, news_feat], dim=-1)            
                proj = self.fusion_proj(proj)                            # [B, T, hid]
            elif self.fusion_type == "crossattn":
                x = proj
                for layer in self.cross_attn_layers:
                    x, _ = layer(x, news_feat)
                proj = x
            elif self.fusion_type == "decoder":
                news_hidden = self.news_fc(news_feat)
                proj = self.decoder_layers(proj, news_hidden)


        proj = self.act(proj)
        rnn_out, _ = self.rnn(proj)  # [batch, seq_len, num_directions * hidden_size]
